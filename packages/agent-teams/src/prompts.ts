...
- Magic placeholders (don't declare in params): \`:__user_id\` (the caller), \`:__now\` (ms epoch), \`:__uuid\` (a new id). Use \`"requires_auth": true\` for writes and user-scoped reads. Only genuinely public read-only \`query\` tools may use \`"requires_auth": false\`, and they must avoid \`:__user_id\`, avoid roles, and include a literal \`LIMIT 500\` or lower. Scope per-user rows with \`WHERE user_id = :__user_id\`.
