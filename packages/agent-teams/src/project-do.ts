...
export class ProjectDO implements DurableObject {
  ...
  // Thin wrappers over ./kb-shares.ts (free functions over SqlStorage).
  private listShares(): Response {
    return listShares(this.state.storage.sql);
  }

  private async createShare(request: Request): Promise<Response> {
    const body = (await request.json()) as CreateShareInput;
    return createShare(this.state.storage.sql, body);
  }

  private revokeShare(shareId: string): Response {
    return revokeShare(this.state.storage.sql, shareId);
  }

  private accessKbViaShare(shareId: string): Response {
    return accessKbViaShare(this.state.storage.sql, shareId);
  }

  private accessKbFileViaShare(shareId: string, filePath: string): Response {
    return accessKbFileViaShare(this.state.storage.sql, shareId, filePath);
  }
  ...
  private handleQAChat(request: Request): Promise<Response> {
    return handleQAChat({
      sql: this.state.storage.sql,
      env: this.env,
      touchUserActivity: () => this.touchUserActivity(),
