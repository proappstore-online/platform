...
|---|---|---|---|
| Scheduled / background execution (reapers, digests, reminders, nightly jobs) | supported for bounded registered actions | an idempotent, `LIMIT`-bounded `execute` / `batch` with fixed `schedule` params, an authenticated unscoped reason and a ≥5-minute UTC cron; inspect `list_scheduled_runs` | [PAS-DATA-019](https://docs.proappstore.online/standard/data/#pas-data-019); platform issue #123 |
